...
//    }
    ...
    while (1)
    {
        ...
        if (err)
        {
            if (bleChar == 0xAA) //脚踏抬起
            {
                //Ble.state.bit.link = 0;
                bsp_PutKey(PEDAL_UP);
            }else
            if (bleChar == 0x55) //脚踏踩下
            {
...
//        }
        ...
        // if (Ble.state.bit.link) //更新BLE联网图标状态
        // {
...
static void vTaskMotorCtrl(void *pvParameters)
{
    ...
    while(1)
    {
		...
		if(lg_lcd_init == 0) continue; //等待LCD初始化完成
		    
		key_v = bsp_GetKey();
        //if( LCD_GetScreenId() == 0 ) //主界面一
        {
            switch( key_v ) //
    		{
    			...
    			// || ( Tmy.speed == SLOW    ) ) //常慢速
    			{
    				if( timer_pedal_up > DLY_PEDAL_UP_MOTOR_STOP ) //抬起脚踏时间大于 DLY_PEDAL_UP_MOTOR_STOP 秒
    				{
    					...
    					if( ( Tmy.Key.hx ) //回吸模式有效
    					  &&( ( Tmy.speed == SLOWEST ) || ( Tmy.speed == SLOW ) ) )//慢慢速 常慢速
    					{
    						if(!lg_zdgy) //非自动给药
    						{
    							lg_zdgy = 0;//自动给药抬起标志，取消
    							Motor.counter_hx_step_up = Motor.counter_step; //存储回吸位置
        ...
        //                    }
                ...
                case DG_STOP_BOT : //停留在底部
                    ...
                    if( key_v == PEDAL_DOWN ) //检测到踩脚踏
                    {
                        ...
                        if( Tmy.Key.ecpk ) //二次排空
                        {
                           // SetTextValue(0,26,"二次排空...");
                            dg_sta = DG_ECPK_UP;//顶杆上升
                        }else
    ...
    //                }
                ...
                case DG_STOP : //停止
                    ...
                    if( dg_sta_c != dg_sta )
                    {
                        ...
                        if (dev_sta == DEV_ERR) //设备故障
                        {
                            SetTextValue(0,26,"零位传感器故障");
                        }
                    }
...
static void AppTaskCreate (void)
{
    ...
    xTaskCreate( vTaskAdc,   	/* 任务函数  */
                 "vTaskAdc",     	/* 任务名    */
                 STKSIZE_TaskAdc,               	/* 任务栈大小，单位word，也就是4字节 */
                 NULL,              	/* 任务参数  */
                 PRIO_TaskAdc,                 	/* 任务优先级*/
                 &xHandleTaskAdc );  /* 任务句柄  */
