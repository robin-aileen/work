...
//         *( msg + i++) = 0x00; //IDL
//         *( msg + i++) = 0x00; //IDH
//         *( msg + i++) = 0;//!Lock.state.bit.lock; //STA 是锁的开关状态
...
//         *( msg + i++) = 0x01; //LEN 后续有效字节数
//         *( msg + i++) = 0;//xxx Lock.power_percent; //电池电量百分比 0~100
...
//         *( msg + i++) = 0x01; //LEN 后续有效字节数
//         *( msg + i++) = 0;//xxx !Lock.state.bit.lock; //STA 为锁状态， 00 表示开启状态， 01 表示关闭状态。 这里与锁的协议相反
...
//     BLE_TxBuf[ i++ ] = 0;//xxx !Lock.state.bit.lock; //STA 为锁状态， 00 表示开启状态， 01 表示关闭状态。
...
//    Lock.state.bit.login = 0;
//    Lock.state.bit.link_to_server = 0;
    debug_usr("[Ble : start.]\r\n");
//    test_ble_cmd( );
    BLE_HwReset( );
    BLE_PapaInit( );
    ...
    //SoftWdtInit( vTaskOtg_ID, 5000 );
    
    Ble.timer_ble_nodata = BLE_TIME_NODATA;
    
//    lock_sta = Lock.state.bit.lock;
    ...
    while( 1 )
    {
        ...
        //蓝牙通信部分 可以考虑单独开个任务---------------------------------------------------------
        if( ( !Ble.timer_ble_nodata ) && ( !Ble.state.bit.sleep ) )
        {
            Ble.timer_ble_nodata = BLE_TIME_NODATA; 
            Ble.state.bit.go_to_sleep = 1;
        }
        
        if( ( Ble.state.bit.sleep == 0 ) && Ble.state.bit.go_to_sleep )
        {
            debug_usr("[ble go to sleep]\r\n");
            
            Ble.state.bit.go_to_sleep = 0;
            Ble.state.bit.wake = 0;
            BLE_Sleep( );
            Ble.state.bit.sleep = 1;
        }
        
        if( ( Ble.state.bit.wake == 0 ) && Ble.state.bit.go_to_wake )
        {
            debug_usr("[ble go to wake]\r\n");
        ...
        //if( Ble.state.bit.link ) //蓝牙已连接
        {
//            if( lock_sta != Lock.state.bit.lock )//锁状态上报
//            {
//                debug_usr("[ble lock sta changed]\r\n");
//                lock_sta = Lock.state.bit.lock;
//                if( Lock.state.bit.lock ) //关锁
//                {
//                    debug_usr("[ble unlock notice]\r\n");
//                    //蓝牙上报关锁状态
//                    comSendBuf( BLE_COM, "\x05\x08\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16 );
//                }
//            }
        }
...
// #define PRIO_TaskBle                6
// #define STKSIZE_TaskBle             128
// static void vTaskBle(void *pvParameters);
// static TaskHandle_t xHandleTaskBle = NULL;

// xTaskCreate( vTaskBle,     		    /* 任务函数  */
//                  "vTaskBle",   		    /* 任务名    */
//                  STKSIZE_TaskBle,        /* 任务栈大小，单位word，也就是4字节 */
//                  NULL,           		    /* 任务参数  */
//                  PRIO_TaskBle,           /* 任务优先级*/
//                  &xHandleTaskBle );      /* 任务句柄  */
