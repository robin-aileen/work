...
qsize queue_find_cmd(qdata *buffer,qsize buf_len)
{
    ...
    //if( comGetLen(COM1) > 3 ) //收接收数据
    {
        while( comGetLen(ULCD_COM) )
        {
            comGetChar(ULCD_COM, &tem8);
            if( i == 0 )
            {
                if( tem8 == 0xEE ) 
                {
...
//    cmd_pos = 0;
//    
//    if( comGetLen(COM1) < 3 ) return 0;
//        
//    while(comGetLen(COM1)>0)//while(queue_size()>0)
//    {
//        //取一个数据
//        comGetChar( COM1,&_data );//移植代码//queue_pop(&_data);
//        
